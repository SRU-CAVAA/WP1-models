...
def beta_variance(a: float | np.ndarray, b: float | np.ndarray) -> float | np.ndarray:
    """
...
class RLagent:
    ...
    def __entropy__(self, s: int, a: int, mode: str) -> float | None:
        ...
        if mode not in ['trans']:
            raise ValueError('Only transition entropy can be computed this way.')
    ...
    def __temporal_difference_error__(self, s: int, a: int, s_prime: int, rew: np.ndarray) -> float | np.ndarray:
        """
    ...
    def __value_iteration__(self, s: int, a: int) -> float | np.ndarray:
        """
    ...
    def model_learning(self, s: int, a: int, s_prime: int, r: float) -> Tuple[float, float]:
        ...
        alpha = np.sum(curr_Rhist[0:curr_V + 1] != 0)
        beta = np.sum(curr_Rhist[0:curr_V + 1] == 0)
    ...
    def inference(self, s: int, a: int, s_prime: int, rew: np.ndarray, **kwargs) -> float | None:
        """
